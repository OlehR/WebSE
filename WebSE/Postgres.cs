...
namespace WebSE
{
    ...
    {       
        ...
        public Postgres()
        {
            try
            {
                string PGInit = Startup.Configuration.GetValue<string>("PGInit");
        ...
        public Utils.Status SaveReceipt(Receipt pR)
        {
            ...
            }catch (Exception e) { return new Utils.Status(e); }
 ...
 values (@IdWorkplace, @CodePeriod,@CodeReceipt, @CodeWares, @CodeUnit, @Quantity, @TypeDiscount, @Sum, @CodePS, @NumberGroup, @BarCode2Category, @TypeWares);";
                ...
                catch (Exception e)
                {
                    ...
                    Con.Execute($@"update ""LogInput"" set ""State"" =1, ""CodeError"" = -1, ""Error"" = @Error where ""Id""=@Id ", new {  Id, Error = e.Message });
                    //return new Utils.Status(e);
                }
            });
            return new Utils.Status();
        }
        ...
        {
            var res = Con.Query<ExciseStamp>(@"select * from ""ExciseStamp"" where ""Stamp""=@Stamp",pES);

        ...
        static public async System.Threading.Tasks.Task<Status<string>> RequestAsync(string pUrl,string pBody,int parWait=1000,string pContex= "text/xml",string pAuth=null)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    return new Status<string>() { Data = res };
                }
                else
                {
                    return new Status<string>((int)response.StatusCode, response.StatusCode.ToString()) { Data = res };
            ...
            {
                return new Status<string>(-1, e.Message);
            }

...
,dn.name_full AS description,try_convert(int,b.code_brand) as meker_id, CASE WHEN p.Code_wares is not NULL THEN 'https://api.spar.uz.ua/Wares/'+dn.code+'.png' end AS image
 FROM dbo.V1C_dim_nomen dn
...
,0x80DA000C29F3389511E7E3CD1E441571,0x831B001517DE370411DFA46F147AE02D,0x81740050569E814D11EBDF2B5D2FA879,0x81960050569E814D11EC89AD29872591)";
        ...
        {
            RestSU RestSU = new();
            var sql = @"select JSON from dbo.PriceJSON";
            var Data = connection.Query<string>(sql);
            var d = Data.Select(x=>   JsonConvert.DeserializeObject<WaresPrice>(x));
            RestSU.residue = d?.Select(x => new ResidueSU(x));
            return RestSU;
        }
    }
}
